...
// ===== Supabase (client criado no index.html) =====
...
async function syncFromDB() {
  try {
    ...
    employeesDB = emps || [];
    console.log("✅ Funcionários carregados:", employeesDB.length, employeesDB);
...
async function dbUpsertLeave(employeeId, fridayBR, status="Pendente", notes="Aguardando aprovação da gestão") {
  ...
  // Apaga qualquer registro anterior para o mesmo funcionário + data
  await sb
    .from("folgas")
    .delete()
  ...
  const { error } = await sb
    .from("folgas")
    .insert({ employee_id: employeeId, friday_date: fridayISO, status, notes });
...
// ===================== ESTATÍSTICAS PÁGINA COLABORADORES =====================
