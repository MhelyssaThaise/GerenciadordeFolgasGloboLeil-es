...
// Preenche os <select> de departamento (add + edit) apenas com a lista oficial
function populateDepartmentSelects() {
  const addSel  = document.getElementById('employeeDepartmentPage');
  const editSel = document.getElementById('editEmployeeDepartment');

  [addSel, editSel].forEach(sel => {
    if (!sel) return;
    const current = sel.value;        // tenta preservar o selecionado
    sel.innerHTML = "";
    ...
    if (current && DEPARTMENTS.includes(current)) sel.value = current;
  });
}
