/* =========================================================
   ...
   funcionarios:
     id (bigint, PK) | name text | email text | department text | photo_url text | created_at timestamptz default now()

   folgas:
     id (bigint, PK) | employee_id bigint (FK) | friday_date date | status text | notes text | created_at timestamptz default now()
   ========================================================= */
...
let pendingDeletionImpactCount = 0;
let pendingDeletionFridays = [];
let pendingRemoveFromFridayId = null;

// Estado em memória (sincronizado com DB)
let fridayData = {};     // { 'DD/MM/YYYY': [ {id,globalId,name,department,status,notes} ] }  (id = id da folga)
let employeesDB = [];    // [{ id, name, email, department, photo_url, created_at }]
// ========================================================


// ===================== SUPABASE =========================
const sb = window.supabase; // já criado no HTML
...
async function dbUpdateEmployee(id, { name, email, department, photo }) {
  ...
  const { error } = await sb.from('funcionarios').update(payload).eq('id', id);
  if (error) throw error;
  await syncFromDB();
}
...
async function dbUpsertLeave(employeeId, fridayBR, status='Pendente', notes='Aguardando aprovação da gestão') {
  ...
  // precisa ter UNIQUE (employee_id, friday_date) na tabela para onConflict
  const { error } = await sb.from('folgas')
    .upsert(
...
// Desativa as antigas funções de localStorage (não usadas)
...
function updateStats() {
  ...
  const elTotal = document.getElementById('totalEmployees'); if (elTotal) elTotal.textContent = totalEmployeesInSystem;
  ...
  const elHeader = document.getElementById('headerPendingCount'); if (elHeader) elHeader.textContent = totalPending;
}

function getStatusBadge(status) {
  if (status === 'Folga')
    return '<span class="status-badge inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">✅ Folga</span>';
  if (status === 'Pendente')
    return '<span class="status-badge inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800">⏳ Pendente</span>';
  if (status === 'Rejeitada')
    return '<span class="status-badge inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800">❌ Rejeitada</span>';
  return '<span class="status-badge inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-orange-100 text-orange-800">💼 Trabalhando</span>';
}
...
function renderEmployees() {
  ...
  employees.forEach(empRow => {
    ...
    const photoDisplay = (glob?.photo_url)
      ...
      : `<div class="w-10 h-10 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full flex items-center justify-center text-white font-semibold">${empRow.name.charAt(0)}</div>`;
    ...
    tr.innerHTML = `
      <td class="py-4 px-6">
        <div class="flex items-center">
          ${photoDisplay}
          <div class="ml-3"><div class="font-medium text-gray-900">${empRow.name}</div></div>
        </div>
      </td>
...
function removeEmployee(leaveId){
  ...
  // solicita senha (como você queria apenas na tela inicial)
...
function renderRemoveFromFridayConfirm(){
  ...
  showInfoModal('🗑️ Remover Colaborador',
...
function showAddEmployeeForm(){ document.getElementById('addEmployeeFormSection')?.classList.remove('hidden'); }
...
function confirmDeleteEmployee(id){
  ...
  if(!emp){ showInfoModal('❌ Erro','Colaborador não encontrado.'); return; }

  // Sextas afetadas (mês atual em memória)
  const details = [];
  let count = 0;
  Object.keys(fridayData).forEach(k=>{
    if((fridayData[k]||[]).some(r=>r.globalId===emp.id)){ details.push(k); count++; }
  });
...
async function executeConfirmedDeletion(){
  ...
  pendingDeletionId = null; pendingDeletionEmployee = null;
}
...
function renderPendingRequests(){
  ...
  const html = list.map(emp=>`
    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-3">
      ...
      <div class="text-sm text-gray-600">${emp.department}</div>
      <div class="text-sm text-gray-700 mt-1"><b>Sexta:</b> ${emp.friday}<br><b>Obs.:</b> ${emp.notes||'-'}</div>
    </div>`).join('');
  showInfoModal('📋 Solicitações Pendentes', `<div class="max-h-96 overflow-y-auto">${html}</div>`);
}

// ======= Form handlers / inputs =======
...
document.getElementById('editEmployeeForm')?.addEventListener('submit', async (e)=>{
  e.preventDefault();
  if (!editingEmployeeId){ showInfoModal('❌ Erro','Nenhum colaborador selecionado.'); return; }

  const name  = (document.getElementById('editEmployeeName')?.value || '').trim();
  const email = (document.getElementById('editEmployeeEmail')?.value || '').trim();
  let department = document.getElementById('editEmployeeDepartment')?.value || '';
  if (department === 'custom'){
    const custom = (document.getElementById('customDepartmentEdit')?.value || '').trim();
    if (!custom){ showInfoModal('⚠️ Atenção','Digite o novo departamento.'); return; }
    department = custom;
  }
...
document.getElementById('addEmployeeFormPage')?.addEventListener('submit', async (e)=>{
  ...
  let department = document.getElementById('employeeDepartmentPage')?.value || '';
  if (department === 'custom'){
    const custom = (document.getElementById('customDepartmentPage')?.value || '').trim();
    if (!custom){ showInfoModal('⚠️ Atenção','Digite o novo departamento.'); return; }
    department = custom;
  }
...
document.getElementById('registerLeaveForm')?.addEventListener('submit', async (e)=>{
  e.preventDefault();
  if (!selectedFriday){ showInfoModal('⚠️ Atenção','Selecione uma sexta-feira.'); return; }
  const employeeId = parseInt(document.getElementById('leaveEmployeeSelect')?.value || '0', 10);
  if (!employeeId){ showInfoModal('⚠️ Atenção','Selecione um colaborador.'); return; }
...
document.getElementById('employeePhotoPage')?.addEventListener('change', (e)=>{
  ...
  const prev = document.getElementById('photoPreviewPage');
  if (!file){ prev.innerHTML = '<span class="text-gray-400 text-2xl">📷</span>'; return; }
  const r = new FileReader();
...
// Departamento custom (add/edit)
document.getElementById('employeeDepartmentPage')?.addEventListener('change', (e)=>{
  const ci = document.getElementById('customDepartmentPage');
  if (!ci) return;
  if (e.target.value==='custom'){ ci.classList.remove('hidden'); ci.required = true; ci.focus(); }
  else { ci.classList.add('hidden'); ci.required = false; ci.value=''; }
});
document.getElementById('editEmployeeDepartment')?.addEventListener('change', (e)=>{
  const ci = document.getElementById('customDepartmentEdit');
  if (!ci) return;
  if (e.target.value==='custom'){ ci.classList.remove('hidden'); ci.required = true; ci.focus(); }
  else { ci.classList.add('hidden'); ci.required = false; ci.value=''; }
});
...
function renderMonthSelect(){
  ...
  months.forEach((nm,idx)=>{
    const o = document.createElement('option');
    o.value = idx; o.textContent = nm; if (idx===currentMonth) o.selected = true;
    sel.appendChild(o);
  });
}
async function selectMonth(month, year){
  ...
  currentYear  = year;
  selectedFriday = null;
  const cv = document.getElementById('currentView'); if (cv) cv.textContent = `${['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'][month]} ${year}`;
  await syncFromDB();
  updateTableTitle();
}
function getFridaysInMonth(year, month){
  ...
  while (dt.getMonth() === month){ fridays.push(new Date(dt)); dt.setDate(dt.getDate()+7); }
  return fridays;
}
...
function updateTableTitle(){
  ...
  t.textContent = selectedFriday ? `Colaboradores - ${selectedFriday}` : 'Colaboradores - Selecione uma sexta-feira';
}
...
function updateEmployeesPageStats(){
  ...
  const todayCount = employeesDB.filter(e=>new Date(e.created_at).toDateString()===today).length;
...
function renderEmployeesList(){
  ...
  employeesDB.forEach(emp=>{
    const photo = emp.photo_url
      ...
      : `<div class="w-10 h-10 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full flex items-center justify-center text-white font-semibold">${emp.name.charAt(0)}</div>`;
    const tr = document.createElement('tr');
    tr.className = 'table-row border-b border-gray-100';
    tr.innerHTML = `
      <td class="py-4 px-6">
        <div class="flex items-center">${photo}<div class="ml-3"><div class="font-medium text-gray-900">${emp.name}</div></div></div>
      </td>
      <td class="py-4 px-6 text-gray-700">${emp.email||''}</td>
      <td class="py-4 px-6 text-gray-700">${emp.department||''}</td>
      <td class="py-4 px-6 text-gray-600">${new Date(emp.created_at).toLocaleDateString('pt-BR')}</td>
      <td class="py-4 px-6 text-center">
...
function populateEmployeeSelect(){
  ...
  const sorted = [...employeesDB].sort((a,b)=>a.name.localeCompare(b.name,'pt-BR'));
  sorted.forEach(e=>{
    const o = document.createElement('option');
    o.value = e.id; o.textContent = `${e.name} - ${e.department||''}`;
...
Object.assign(window,{
  ...
  toggleStatus, processToggle,
  approveLeave, rejectLeave, processApproval, processRejection,
  removeEmployee, renderRemoveFromFridayConfirm, executeRemoveEmployee,
  selectFriday, updateAllInterfaces, closeInfoModal,
  openEditEmployeeModal, closeEditEmployeeModal,
  confirmDeleteEmployee, executeConfirmedDeletion,
});
